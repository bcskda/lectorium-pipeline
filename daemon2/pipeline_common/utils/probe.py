...
from pipeline_common.apps import PipelineCommonConfig
...
class VideoFile(namedtuple("VideoFile", "path start duration modification_time".split())):
    ...
    def time_info_str(self):
        fmt = "[{0:11.2f} - {1:11.2f}) {2:11.2f}   [{3:%H:%M:%S} - {4:%H:%M:%S})"
        return fmt.format(self.start, self.end, self.duration, self.start_date.time(), self.end_date.time())
...
def get_video_format_info(path) -> dict:
    ...
    """
    ffprobe_cmd = [
        PipelineCommonConfig.ffprobe_path,
...
def extract_groups(videos: List[VideoFile]) -> List[List[VideoFile]]:
    ...
    for group in groups:
        first = group[0]
        if first.start >= 0.5:
            fmt = "Warning: first group element ({0!r}) has large start ({1:.2f}), maybe change grouping strategy?"
            logging.info(fmt.format(first.path, first.start))
